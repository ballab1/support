...
function term.decode()
{
    ...
    case "$name" in
         task)     term.get_code ${FOREGROUND[lt_cyan]};;
         info)     term.get_code ${FOREGROUND[lt_green]};;
         warn)     term.get_code ${FOREGROUND[lt_yellow]};;
        error)     term.get_code ${FOREGROUND[red]};;
        reset)     term.get_code ${ATTR_RESET[all]};;
            *)     term.get_code ${FOREGROUND[$name]};;
    esac
}

############################################################################# 
function term.get_fb()
{
